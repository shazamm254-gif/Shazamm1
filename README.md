...
| `tools/to_verticals.py` | **The render bridge** — converts generated scripts into [Verticals v3](https://github.com/rushindrasinha/youtube-shorts-pipeline) draft JSON (+ a matching niche profile), so an open-source pipeline renders and uploads them. Refuses scripts with unfilled `[FACT: ...]` slots, which TTS would read aloud. |
| `tools/kokoro_vo.py` | **The voiceover step** — records a draft's script locally with [Kokoro](https://github.com/hexgrad/kokoro) (82M params, Apache-2.0, CPU, no API key) and hands the audio to the pipeline. Graded voice list; warns before using a poor one. |
...
| [`docs/VISUAL_PACK_SCAMS.md`](docs/VISUAL_PACK_SCAMS.md) | **Shot-by-shot AI image prompts for all 10 scripts** — 46 prompts with edit timings, motion notes and thumbnails, two master style suffixes (modern / historical lanes), 4 reusable assets that serve the whole back catalogue, and the four rules that keep generated imagery legally and ethically safe. |
| [`docs/THUMBNAIL_CHECKLIST_SCAMS.md`](docs/THUMBNAIL_CHECKLIST_SCAMS.md) | First-frame / thumbnail checklist tuned to this niche — the "one ordinary object, one thing wrong" rule that replaces a face, the three text shapes that work, per-script thumbnail specs for the top 10, and the niche-specific hazards (looking like a scam ad, real brand marks, the hooded-hacker clichés). |
